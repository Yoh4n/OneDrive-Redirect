...
$UserEMail  = $env:USERNAME + $TenantDomain
$OneDrivePath = "$env:USERPROFILE\OneDrive - $TenantName"

forEach($Kvp in $RegName.GetEnumerator()) {

$TestValue = Test-RegistryValue -Path $RegPath -Value $kvp.Value
$RegValue = "%USERPROFILE%\OneDrive - $TenantName\" + $kvp.key

    if($TestValue -eq "False" ) {
    
        # Set sustem Known Folder Path
        Set-KnownFolderPath -KnownFolder $kvp.Key -Path ("$OneDrivePath\" + $kvp.Key)

        # Move contents to OneDrive folder
        Move-Item -Path ("$env:USERPROFILE\" + $kvp.Key + "\*") -Destination ("$OneDrivePath\" + $kvp.key) -Force -Verbose

        # Create redirection registry value
        New-ItemProperty -Path $RegPath -Name $Kvp.value -Value $RegValue -PropertyType ExpandString -Force

    } else {
        
        # Set existing registry value to redirected folder.
        Set-ItemProperty -Path $RegPath -Name $Kvp.value -Value $RegValue

    }

   }

#Restart Explorer.exe to reflect registry changes.
